package ca.landonjw.remoraids.internal.inventory.api;
 ...
 *
 * <p>
 * Each page has a {@link Template} that defines the layout of the page,
 * and a page may have many different {@link Button}s that make the page interactive.
 *
 * In order to send a page to a player, use {@link #openPage(EntityPlayerMP)} or {@link #forceOpenPage(EntityPlayerMP)}.
 * </p>
 *
 * @author landonjw
 * @since 1.0.0
...
public interface Page {
	 ...
	 *
	 * <p>
	 * If the player is currently in an inventory, this will wait until that inventory
	 * is closed before opening. If you want to forcibly open the page, use
	 * {@link #forceOpenPage(EntityPlayerMP)} instead.
	 * </p>
	 *
	...
	interface Builder {
		 ...
		 *
		 * <p>
		 * This can be used alongside {@link #dynamicContents(List)} and will generate
		 * as many pages as required to fix the size of the contents.
		 * </p>
		 *
		 * <p>
		 * If there are dynamic contents set and this isn't defined with a valid space,
		 * invoking {@link #build()} will result in an {@link IllegalStateException}.
		 * </p>
		 *
		 ...
		 *
		 * <p>
		 * This can be used alongside {@link #dynamicContentArea(int, int, int, int)} to generate
		 * as many pages as required to fix the size of the contents.
		 * </p>
		 *
		 * <p>
		 * If this is set, a valid dynamic content area must be set or invoking {@link #build()}
