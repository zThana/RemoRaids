...
public class SpawnAnnouncement implements ISpawnAnnouncement {
    ...
    private String getParsedAnnouncement(@Nonnull String announcement, @Nonnull IBossSpawner spawner){
        ...
        IParsingContext context = IParsingContext.builder()
                ...
                .add(Vec3d.class, () -> new Vec3d(
                        spawner.getSpawnLocation().getLocation().x,
                        spawner.getSpawnLocation().getLocation().y,
                        spawner.getSpawnLocation().getLocation().z)
                )
                .build();
    ...
    public JObject serialize() {
        return new JObject()
                .add("message", this.announcement)
                .when(this, x -> x.getTeleport().isPresent(), o -> o.add("teleport", this.getTeleport().get().serialize()));
    }
    ...
    public static class SpawnAnnouncementBuilder implements ISpawnAnnouncementBuilder {
        ...
        public ISpawnAnnouncementBuilder deserialize(JsonObject json) {
            ...
            if(json.has("teleport")) {
                this.teleport = new Teleport(
                        Arrays.stream(FMLCommonHandler.instance().getMinecraftServerInstance().worlds)
                        ...
                        new Vec3d(json.get("x").getAsDouble(), json.get("y").getAsDouble(), json.get("z").getAsDouble()),
                        json.get("rotation").getAsFloat()
                );
            }
            return this;
        }
