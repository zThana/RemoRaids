...
public class AddRewardUI extends BaseBossUI {
	...
	public void open() {
		...
		// IMessageService service = IBossAPI.getInstance().getRaidRegistry().getUnchecked(IMessageService.class); - Never used, no need to create it
		...
		for (ICreatorUI<IReward> creator : IBossUIRegistry.getInstance().getRewardCreators()) {
			Button creatorButton = Button.builder().item(creator.getCreatorIcon()).displayName(creator.getCreatorTitle()).onClick(() -> creator.open(this, player, bossRewardsList)).build();
		...
		Page page = Page.builder().template(template).dynamicContentArea(2, 2, 1, 5).dynamicContents(creatorButtons).title(config.get(MessageConfig.UI_BATTLES_ADD_REWARD_TITLE)).build();
